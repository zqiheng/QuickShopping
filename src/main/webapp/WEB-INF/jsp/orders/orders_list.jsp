...
<div>
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 订单管理 <span
            class="c-gray en">&gt;</span> 所有订单<a class="btn btn-success radius r"
            ...
            <table class="table table-border table-bordered table-bg table-hover table-sort">
                ...
                <c:forEach items="${orders}" var="order">
                    <tr class="text-c va-m">
                        ...
                        <td style="color:#1296db;">${order.pickMode eq true?"自提":"派送"} </td>
                        <td style="color:red;">
                            <c:if test="${order.ordersType eq 0}">已完成</c:if>
                            <c:if test="${order.ordersType eq 1}">待自提</c:if>
                            <c:if test="${order.ordersType eq 2}">待配送</c:if>
                            <c:if test="${order.ordersType eq 3}">待收货</c:if>
                            <c:if test="${order.ordersType eq 4}">已取消</c:if>
                        </td>
                        <td class="text-l">${order.ordersRemark }</td>

                        <td class="td-manage">
                            <a style="text-decoration:none;color: red" class="ml-5" onclick="shop_edit('订单详情','/web/orders/get_orders_detail_info?ordersID='+'${order.ordersID}','4','','510')"
                               href="javascript:;" title="订单详情"> 订单详情</a>
